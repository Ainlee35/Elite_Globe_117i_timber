  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        const data = await apiRequest<Product[]>(`/products${query ? `?${query}` : ""}`);
        setProducts(data);
      } catch {
        setProducts([]);
      } finally {
        ...
        <div className="container relative z-10">
          ...
          >
            ...
            <h1 className="mt-6 font-heading text-4xl font-bold leading-tight md:text-5xl lg:text-6xl">
              Build with <span className="text-primary">Quality</span>, Build with <span className="text-primary">Confidence</span>
            </h1>
          ...
          {CATEGORIES.map(cat => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${
                category === cat.value ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
              }`}
            >
              {cat.label}
            </button>
