...
export default function Orders() {
  const { user } = useAuth();
  const { myOrders, fetchMyOrders } = useOrders();
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-3">
                        <h3 className="font-heading text-lg font-bold">{order.orderCode}</h3>
                        <span className={`flex items-center gap-1 rounded-full px-3 py-1 text-xs font-semibold ${config.bg} ${config.color}`}>
                          <StatusIcon className="h-3 w-3" />
                          {config.label}
                        </span>
                        <span className={`rounded-full px-3 py-1 text-xs font-semibold ${order.paymentStatus === "paid" ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}>
                    ...
                    <p className="font-heading text-xl font-bold text-primary">TZS {order.totalAmount.toLocaleString()}</p>
                  </div>
                  ...
                  <div className="mt-4 border-t pt-4">
                    {order.items.map((item, i) => (
                      <div key={i} className="flex items-center justify-between py-1 text-sm">
                        <span>
                          {item.productName}
                          {item.selectedType && <span className="text-muted-foreground"> ({item.selectedType})</span>} ×{item.quantity}
                        </span>
                        <span className="font-medium">TZS {item.lineTotal.toLocaleString()}</span>
                      </div>
                    ))}
                  </div>
                </motion.div>
